...
public class AdminResultsActivity extends AdminBaseActivity {
    ...
    private void load(){all.clear();final int[] pending={2};final boolean[] failed={false};ValueEventListener listener=new ValueEventListener(){public void onDataChange(DataSnapshot s){for(DataSnapshot x:s.getChildren())all.add(x);if(--pending[0]==0)filter("");}public void onCancelled(DatabaseError e){if(!failed[0]){failed[0]=true;if(empty!=null){empty.setText("Could not load attempts: "+e.getMessage());empty.setVisibility(View.VISIBLE);}toast("Could not load attempts: "+e.getMessage());} }};db.child("quiz_results").addListenerForSingleValueEvent(listener);db.child("tournament_attempts").addListenerForSingleValueEvent(listener);}
    private void filter(String query){shown.clear();for(DataSnapshot s:all){String text=String.valueOf(s.getValue()).toLowerCase();if(text.contains(query.toLowerCase()))shown.add(s);}if(adapter!=null)adapter.notifyDataSetChanged();if(empty!=null)empty.setVisibility(shown.isEmpty()?View.VISIBLE:View.GONE);}
